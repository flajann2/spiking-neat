...
data NeuronState = NeuronState { nV :: !Float, nU :: !Float }

defaultA, defaultB, defaultC, defaultD, defaultI :: Float
defaultA = 0.02  -- recovery time scale
defaultB = 0.2   -- sensitivity of u to subthreshold v
defaultC = -65.0 -- post-spike reset value of v
defaultD = 8.0   -- post-spike bump to u
defaultI = 10.0  -- injected/synaptic current

initialNeuronState :: NeuronState
initialNeuronState = NeuronState defaultC (defaultB * defaultC)
...
main :: IO ()
main = withWindow 2560 1600 "3D Phase Portrait - Izhikevich Neuron, Delay Embedding" 60 $ \_ -> do
  ...
  aRef          <- newIORef defaultA
  bRef          <- newIORef defaultB
  cRef          <- newIORef defaultC
  dRef          <- newIORef defaultD
  iRef          <- newIORef defaultI
