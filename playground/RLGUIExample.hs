...
defaultSigma, defaultRho, defaultBeta, dt :: Float
defaultSigma = 10.0
defaultRho   = 28.0
defaultBeta  = 8.0 / 3.0
dt           = 0.005

stepLorenz :: Float -> Float -> Float -> Vec3 -> Vec3
stepLorenz sigma rho beta (Vec3 x y z) =
  let dx = sigma * (y - x)
      dy = x * (rho - z) - y
      dz = x * y - beta * z
...
main = withWindow 2560 1600 "3D Phase Portrait - Lorenz Attractor, Mouse control" 60 $ \_ -> do
  ...
  sigmaRef      <- newIORef defaultSigma
  rhoRef        <- newIORef defaultRho
  betaRef       <- newIORef defaultBeta
  let maxPoints = 15000 :: Int

  whileWindowOpen0 $ do
    sigma <- readIORef sigmaRef
    rho   <- readIORef rhoRef
    beta  <- readIORef betaRef

    pos <- readIORef currentPosRef
    let newPos = stepLorenz sigma rho beta pos
    ...
    drawing $ do
      ...
      -- GUI overlay: sigma/rho/beta sliders + redraw button
      guiLabel (Rectangle 20 110 200 20) "Lorenz Parameters"

      (_, sigma') <- guiSlider (Rectangle 100 135 200 20) (Just "sigma") (Just (showF sigma)) sigma 0.0 50.0
      writeIORef sigmaRef sigma'

      (_, rho') <- guiSlider (Rectangle 100 165 200 20) (Just "rho") (Just (showF rho)) rho 0.0 50.0
      writeIORef rhoRef rho'

      (_, beta') <- guiSlider (Rectangle 100 195 200 20) (Just "beta") (Just (showF beta)) beta 0.0 10.0
      writeIORef betaRef beta'

      redrawClicked <- guiButton (Rectangle 20 230 120 30) (Just "Redraw")
